...
@pytest.mark.parametrize(
    "cls,field,bad",
    [
        # -inf, not +inf, for altitude_bot_m: +inf is already caught by the
        # bot-below-top coherence check below regardless of this fix (any
        # top is < +inf), which would prove nothing about allow_inf_nan.
        # -inf passes that check (it is always < the default top) so it
        # isolates the fix under test.
        (CloudLayer, "altitude_bot_m", float("-inf")),
        (CloudLayer, "altitude_top_m", float("inf")),
        (WindLayer, "altitude_m", float("inf")),
        (WindLayer, "speed_kt", float("inf")),
    ],
)
def test_rejects_infinity_on_fields_with_no_documented_upper_bound(cls, field, bad):
    """altitude_* carries no bound at all, and speed_kt's ge=0.0 alone does
    not exclude +inf -- unlike NaN, which gt=/ge= already reject. Without
    allow_inf_nan=False, infinity would round-trip into a
    <Tag Value="inf" ...> element MSFS cannot parse, reported as a
    successful write."""
    with pytest.raises(ValidationError):
        cls(**{field: bad})
...
@pytest.mark.parametrize("field", ["msl_temperature_k", "aerosol_density"])
def test_preset_rejects_infinity_on_fields_with_no_documented_upper_bound(field):
    """msl_temperature_k's gt=0.0 and aerosol_density's ge=0.0 both admit
    +inf on their own -- same gap as CloudLayer/WindLayer above."""
    with pytest.raises(ValidationError):
        WeatherPreset(name="X", **{field: float("inf")})
...
def test_to_bytes_matches_committed_fixture():
    """Byte-level guard on the structure, following the pattern in
    tests/fixtures/facilities/ -- but not the same strength of guarantee.
    Those fixtures are wire bytes captured once from a live sim, so they
    validate a decode against external reality; expected_storm.WPR was
    generated by this very module, so a round trip against it can only catch
    drift from today's output, not a format that was already wrong when the
    fixture was captured. Regenerate deliberately if the format is
...
def test_prefers_2024_over_2020_for_the_steam_roaming_folder_too(tmp_path, monkeypatch):
    """Steam MSFS 2024 keeps presets under a differently-named roaming folder
    ("Microsoft Flight Simulator 2024") than Steam MSFS 2020 ("Microsoft
    Flight Simulator"). A machine with both installed must resolve to 2024,
    same as the Store-package candidates above."""
    roaming = tmp_path / "roaming"
    steam_2024 = roaming / "Microsoft Flight Simulator 2024" / "Weather" / "Presets"
    steam_2020 = roaming / "Microsoft Flight Simulator" / "Weather" / "Presets"
    steam_2024.mkdir(parents=True)
    steam_2020.mkdir(parents=True)
    monkeypatch.setenv("LOCALAPPDATA", str(tmp_path / "local"))
    monkeypatch.setenv("APPDATA", str(roaming))
    assert find_presets_dir() == steam_2024
