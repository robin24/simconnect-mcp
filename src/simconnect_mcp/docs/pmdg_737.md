...
`variant="pmdg_777"` for the 777. When detection fails, the event/var name is
looked up in both catalogs and the first match wins; if that also fails, the
777 catalog is used as a last-resort default. Every response carries a
`variant_source` field (`"explicit"`, `"detected"`, `"name_match"`, or
`"fallback"`) so a caller can tell a real detection from a guess.
